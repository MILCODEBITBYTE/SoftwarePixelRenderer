...
void Progress()
{
    ...
    XMMATRIX matWP, matWPP;
    matWP = XMMatrixMultiply(matWorld, matProj);
    matWPP = XMMatrixMultiply(matWP, matViewport);
    
    vDir = XMLoadFloat3(&v3Dir);

    vDir= XMVector3TransformCoord(vDir, matWPP);
...
void DrawMesh(CMesh* pMesh, ID2D1RenderTarget* pRender, ID2D1SolidColorBrush* pBrush)
{
    ...
    */
    ...
    XMMATRIX matMWP = XMMatrixMultiply(matModel, g_matRenderWorld);
    matMWP = XMMatrixMultiply(matMWP, g_matRenderProj);

    
    MESH_MODEL* pModels = pMesh->GetModel();
    MESH_MODEL* pGroup = nullptr;
    ...
    for (size_t model_cnt = 0; model_cnt < tModelCount; ++model_cnt)
    {
        ...
        for (size_t cn = 0; cn < tFaceCount; cn++)
        {
            ...
            for (int i = 0; i < iFaceInVertexCount; ++i)
            {
                ...
                vCalc = XMVector3TransformCoord(vCalc, matMWP);
...
BOOL LoadObjFromFile(const wchar_t* szFileName)
{
    ...
    bRes = g_pCube->LoadFromFile(szFileName);
    if (FALSE == bRes)
    {
        g_pCube->Shutdown();
    }
