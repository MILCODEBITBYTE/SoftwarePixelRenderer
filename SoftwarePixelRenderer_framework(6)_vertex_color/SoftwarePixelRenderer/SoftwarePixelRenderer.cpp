...
RENDER_STATE g_RenderState = RENDER_STATE::TEST;
...
BOOL InitializeSystem()
{
    ...
    g_pVtxColorShader->Initialize(g_pD2dSystem->GetBackbuffer());

    g_pBoundboxShader = new CVertexColorShader;
    g_pBoundboxShader->Initialize(g_pD2dSystem->GetBackbuffer());
...
void DrawMesh(CMesh* pMesh, CShader* pShader)
{
    ...
    if (lstClipFacies.size())
    {
        ...
        for (iter = lstClipFacies.begin(); iter != lstClipFacies.end(); ++iter)
        {
            ...
            //Vertex color shader
            pShader->Render(&*iter, pMesh->GetMaterial(), g_RenderState);
        }
...
void DrawBoundbox(CMesh* pMesh, CShader*pShader)
{
    ...
    if (lstClipFacies.size())
    {
        ...
        for (iter = lstClipFacies.begin(); iter != lstClipFacies.end(); ++iter)
        {
            ...
            g_iFaceCount++;
            pShader->Render(&*iter, nullptr, g_RenderState);
        }
