...
void CVertexColorShader::Render(CFace* pFace, CMaterial* pMaterial, CLight *pLight, const RENDER_STATE state, const bool bLight, const bool bZTest, const bool bStencil)
{
	...
	XMVECTOR vCalc, vVW[3], vDir[2], vNormal;
	...
	if (tVtxCnt == 3)
		DrawTriangle(pVertices, nullptr, pLight, pFace->GetMaterialName(),state, bLight, bZTest, bStencil);
	else
		DrawQuad(pVertices,nullptr, pLight, pFace->GetMaterialName(), state, bLight, bZTest, bStencil);
}


void CVertexColorShader::DrawTrilinearInterpolation(BYTE* pBack, const UINT stride, const VERTEX* pVertices[3], CLight *pLight, const bool bLight, const bool bZTest, const bool bStencil)
{
	...
	LIGHT_TYPE LightType = pLight->GetType();
	...
	if (bLight)
	{
		...
		R = BYTE(v4Col.x * v4ColFinal.x * 255.f);
		G = BYTE(v4Col.y * v4ColFinal.y * 255.f);
		B = BYTE(v4Col.z * v4ColFinal.z * 255.f);
		A = 255;
...
void CVertexColorShader::DrawLinearInterpolation(BYTE* pBuffer, const UINT uiStride, const VERTEX* pStart, const VERTEX* pEnd, CLight* pLight,const bool bLight, const bool bZTest, const bool bStencil)
{
	int w = int(m_matViewport._11 * 2.f);
	int h = int(m_matViewport._22 * -2.f);
...
void CVertexColorShader::FillTrilinearInterpolation(BYTE* pBack, const UINT stride, const VERTEX* pVertices[3], CLight* pLight,  const bool bLight, const bool bZTest, const bool bStencil)
{
	int w = int(m_matViewport._11 * 2.f);
	int h = int(m_matViewport._22 * -2.f);
	...
	XMVECTOR vStartCol, vEndCol, vFinalCol;
	...
	function<void(CBresenhamLine&, const WORD, const XMFLOAT4, const XMFLOAT4, const float, const float)> fcLineNextStep = [&](CBresenhamLine& line, const WORD num, const XMFLOAT4 colStart, const XMFLOAT4 colEnd, const float zStart, const float zEnd)
	{
		...
		do
		{
			...
			vStartCol = XMLoadFloat4(&colStart);
			vEndCol = XMLoadFloat4(&colEnd);

			vFinalCol = XMVectorLerpV(vStartCol, vEndCol, XMVectorReplicate(fRate));

			XMStoreFloat4(&dir.col, vFinalCol);
	...
	XMFLOAT4 colStart, colEnd, v4ColFinal;
...
void CVertexColorShader::FillLinearInterpolation(BYTE* pBuffer, const UINT uiStride, const VERTEX* pStart, const VERTEX* pEnd, CLight* pLight, const bool bLight, const bool bZTest, const bool bStencil)
{
	int w = int(m_matViewport._11 * 2.f);
	int h = int(m_matViewport._22 * -2.f);
	...
	float fIntensity = pLight->GetIntensity();
	XMFLOAT4 v4ColFinal, v4ColAmbient, v4ColDiffuse;
	v4ColAmbient = pLight->GetAmbientColor();
	v4ColDiffuse = pLight->GetDiffuseColor();
	...
	do
	{
		...
		if (bLight)
		{
			...
			fR = BYTE(fR * v4ColFinal.x);
			fG = BYTE(fG * v4ColFinal.y);
			fB = BYTE(fB * v4ColFinal.z);
		}
...
void CVertexColorShader::DrawTriangle(VERTEX* pVertices, CMaterial* pMaterial, CLight *pLight, const wstring strMaterialName, const RENDER_STATE state, const bool bLight, const bool bZTest, const bool bStencil)
{
	if (nullptr == m_pScreenBuffer) return;
	...
	*/
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			{
				DrawLinearInterpolation(pBackData, uiBackStride, pVertices, pVertices + 1, pLight, bLight, bZTest, bStencil);
				DrawLinearInterpolation(pBackData, uiBackStride, pVertices + 1, pVertices + 2, pLight, bLight, bZTest, bStencil);
				DrawLinearInterpolation(pBackData, uiBackStride, pVertices + 2, pVertices, pLight, bLight, bZTest, bStencil);
			}
			...
			else if (state == RENDER_STATE::TEST)
			{
				...
				{
					DrawTrilinearInterpolation(pBackData, uiBackStride, Tri, pLight, bLight, bZTest, bStencil);
				}
				else
				{
					DrawLinearInterpolation(pBackData, uiBackStride, pVertices, pVertices + 1, pLight, bLight, bZTest, bStencil);
			...
			else if (state == RENDER_STATE::FLAT)
			{
				if (false == m_bWireframe)
				{
					FillTrilinearInterpolation(pBackData, uiBackStride, Tri, pLight, bLight, bZTest, bStencil);
				}
				else
				{
					FillLinearInterpolation(pBackData, uiBackStride, pVertices, pVertices + 1, pLight, bLight, bZTest, bStencil);
					FillLinearInterpolation(pBackData, uiBackStride, pVertices+1, pVertices + 2, pLight, bLight, bZTest, bStencil);
					FillLinearInterpolation(pBackData, uiBackStride, pVertices+2, pVertices, pLight, bLight, bZTest, bStencil);
				}
			}
...
void CVertexColorShader::DrawQuad(VERTEX* pVertices, CMaterial* pMaterial, CLight *pLight, const wstring strMaterialName, const RENDER_STATE state, const bool bLight, const bool bZTest, const bool bStencil)
{
	if (nullptr == m_pScreenBuffer) return;
	...
	*/
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (state == RENDER_STATE::BOUNDBOX)
			{
				DrawLinearInterpolation(pBackData, uiBackStride, TriA[0], TriA[1], pLight, bLight, bZTest, bStencil);
				DrawLinearInterpolation(pBackData, uiBackStride, TriA[1], TriA[2], pLight, bLight, bZTest, bStencil);
				DrawLinearInterpolation(pBackData, uiBackStride, TriA[2], TriA[0], pLight, bLight, bZTest, bStencil);

				DrawLinearInterpolation(pBackData, uiBackStride, TriB[0], TriB[1], pLight, bLight, bZTest, bStencil);
				DrawLinearInterpolation(pBackData, uiBackStride, TriB[1], TriB[2], pLight, bLight, bZTest, bStencil);
				DrawLinearInterpolation(pBackData, uiBackStride, TriB[2], TriB[0], pLight, bLight, bZTest, bStencil);
			}
			else if (state == RENDER_STATE::TEST)
			{
				if (false == m_bWireframe)
				{
					DrawTrilinearInterpolation(pBackData, uiBackStride, TriA, pLight, bLight, bZTest, bStencil);
					DrawTrilinearInterpolation(pBackData, uiBackStride, TriB, pLight, bLight, bZTest, bStencil);
				}
				else
				{
					DrawLinearInterpolation(pBackData, uiBackStride, TriA[0], TriA[1], pLight, bLight, bZTest, bStencil);
			...
			else if (state == RENDER_STATE::FLAT)
			{
				if (false == m_bWireframe)
				{
					FillTrilinearInterpolation(pBackData, uiBackStride, TriA, pLight, bLight, bZTest, bStencil);
					FillTrilinearInterpolation(pBackData, uiBackStride, TriB, pLight, bLight, bZTest, bStencil);
				}
				else
				{
					FillLinearInterpolation(pBackData, uiBackStride, TriA[0], TriA[1], pLight, bLight, bZTest, bStencil);
					FillLinearInterpolation(pBackData, uiBackStride, TriA[1], TriA[2], pLight, bLight, bZTest, bStencil);
					FillLinearInterpolation(pBackData, uiBackStride, TriA[2], TriA[0], pLight, bLight, bZTest, bStencil);

					FillLinearInterpolation(pBackData, uiBackStride, TriB[0], TriB[1], pLight, bLight, bZTest, bStencil);
					FillLinearInterpolation(pBackData, uiBackStride, TriB[1], TriB[2], pLight, bLight, bZTest, bStencil);
					FillLinearInterpolation(pBackData, uiBackStride, TriB[2], TriB[0], pLight, bLight, bZTest, bStencil);
