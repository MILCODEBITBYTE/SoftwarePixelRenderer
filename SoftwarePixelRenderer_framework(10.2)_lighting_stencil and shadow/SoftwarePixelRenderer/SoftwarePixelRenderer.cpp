...
const int STENCIL_BUFFER_WIDTH = 800;
const int STENCIL_BUFFER_HEIGHT = 600;
...
CTextureSmoothShader* g_pTextureSmoothShader = nullptr;
CStencilShader* g_pStencilShader = nullptr;

CLight* g_pAmbientLight = nullptr;
...
bool g_bStencilFillShow = false;
bool g_bPostZFillShow = false;
bool g_bPostStencilFillShow = false;
bool g_bLightOn = true;
bool g_bFastComputeLight = true;
...
void RenderObject(CGameObject* pObj, CShader* pShader, CLight *pLight);
void StencilRenderObject(CGameObject* pObj, CShader* pShader, const CULL_BACKFACE cull);
void DrawMesh(CMesh* pObj, CShader* pShader);
void StencilDrawMesh(CMesh* pObj, CShader* pShader, const CULL_BACKFACE cull);
void DrawBoundbox(CMesh* pMesh, CShader* pShader);

void ClearObject();
BOOL LoadObjFromFile(const wchar_t* szPth, const wchar_t* szFileName);
...
void BackfaceCulling(CMesh* pMesh, const CULL_BACKFACE enFaceWhere, list<CFace>& lstClipOut, const XMMATRIX &matWorld, const XMMATRIX &matView, const XMMATRIX &matProj);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    CheckMenuItem(hMenu, ID_STENCILBUFFER_ON, MF_CHECKED);
    CheckMenuItem(hMenu, ID_ZFILL_HIDE, MF_CHECKED);
    CheckMenuItem(hMenu, ID_STENCILFILL_HIDE, MF_CHECKED);

    CheckMenuItem(hMenu, ID_RENDER_STATE_TEXTURE, MF_CHECKED);
    CheckMenuItem(hMenu, ID_LIGHT_ON, MF_CHECKED);
    CheckMenuItem(hMenu, ID_LIGHTTYPE_DIRECTIONAL, MF_CHECKED);
    CheckMenuItem(hMenu, ID_LIGHTTYPE_FASTCOMPUTE, MF_CHECKED);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        break;
        case ID_STENCILBUFFER_ON:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            CheckMenuItem(hMenu, ID_STENCILBUFFER_ON, MF_CHECKED);
            CheckMenuItem(hMenu, ID_STENCILBUFFER_OFF, MF_UNCHECKED);
            g_bStencil = true;
        }
        break;
        case ID_STENCILBUFFER_OFF:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            CheckMenuItem(hMenu, ID_STENCILBUFFER_ON, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_STENCILBUFFER_OFF, MF_CHECKED);
            g_bStencil = false;
        }
        break;
        case ID_ZBUFFER_SHOW:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            CheckMenuItem(hMenu, ID_ZBUFFER_SHOW, MF_CHECKED);
            CheckMenuItem(hMenu, ID_STENCIL_SHOW, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_ZFILL_HIDE, MF_UNCHECKED);
            EnterCriticalSection(&g_csRenderList);
            g_bZFillShow = true;
            g_bStencilFillShow = false;
            LeaveCriticalSection(&g_csRenderList);
        }
        break;
        case ID_STENCIL_SHOW:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            CheckMenuItem(hMenu, ID_ZBUFFER_SHOW, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_STENCIL_SHOW, MF_CHECKED);
            CheckMenuItem(hMenu, ID_ZFILL_HIDE, MF_UNCHECKED);
            EnterCriticalSection(&g_csRenderList);
            g_bZFillShow = false;
            g_bStencilFillShow = true;
            LeaveCriticalSection(&g_csRenderList);
        }
        break;
        case ID_ZFILL_HIDE:
        ...
        case ID_LIGHT_ON:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            CheckMenuItem(hMenu, ID_LIGHT_ON, MF_CHECKED);
            CheckMenuItem(hMenu, ID_LIGHT_OFF, MF_UNCHECKED);
            g_bLightOn = true;
        }
        break;
        case ID_LIGHT_OFF:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            CheckMenuItem(hMenu, ID_LIGHT_ON, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_LIGHT_OFF, MF_CHECKED);
            g_bLightOn = false;
        }
        break;
        case ID_LIGHTTYPE_FASTCOMPUTE:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            if(true == g_bFastComputeLight)
                CheckMenuItem(hMenu, ID_LIGHTTYPE_FASTCOMPUTE, MF_UNCHECKED);
            else
                CheckMenuItem(hMenu, ID_LIGHTTYPE_FASTCOMPUTE, MF_CHECKED);

            g_bFastComputeLight = !g_bFastComputeLight;
        }
        break;
        ...
        break;
        case ID_LIGHTTYPE_DIRECTIONAL:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_DIRECTIONAL, MF_CHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_POINT, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_SPOTLIGHT, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_PHONG, MF_UNCHECKED);
            g_pAmbientLight->SetType(LIGHT_TYPE::DIRECTIONAL);
        }
        break;
        case ID_LIGHTTYPE_POINT:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_DIRECTIONAL, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_POINT, MF_CHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_SPOTLIGHT, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_PHONG, MF_UNCHECKED);
            g_pAmbientLight->SetType(LIGHT_TYPE::POINT);
        }
        break;
        case ID_LIGHTTYPE_SPOTLIGHT:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_DIRECTIONAL, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_POINT, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_SPOTLIGHT, MF_CHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_PHONG, MF_UNCHECKED);
            g_pAmbientLight->SetType(LIGHT_TYPE::SPOT);
        }
        break;
        case ID_LIGHTTYPE_PHONG:
        {
            HMENU hMenu = GetMenu(g_hWnd);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_DIRECTIONAL, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_POINT, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_SPOTLIGHT, MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_LIGHTTYPE_PHONG, MF_CHECKED);
            g_pAmbientLight->SetType(LIGHT_TYPE::PHONG);
        }
        break;
        default:
...
BOOL InitializeSystem()
{
    ...
    g_vecStencilBuffer.resize(ZBUFFER_WIDTH * ZBUFFER_HEIGHT);
    ...
    pMesh->LoadFromFile(L"./resources/", L"tex_cube.obj");
    ...
    g_pTextureSmoothShader = new CTextureSmoothShader;
    g_pTextureSmoothShader->Initialize(g_pD2dSystem->GetRenderbuffer());

    g_pStencilShader = new CStencilShader;

    g_pAmbientLight = new CLight();

    XMFLOAT3 v3Pos = XMFLOAT3(0, 5, -5);
   

    g_pAmbientLight->SetPosition(v3Pos);
    g_pAmbientLight->SetRange(10);
    g_pAmbientLight->SetCone(20);
    g_pAmbientLight->SetSpecularPower(20);
    g_pAmbientLight->SetAttenuation(XMFLOAT3(0.2f, 0.02f, 0));
    g_pAmbientLight->SetAmbientColor(XMFLOAT4(0.3f, 0.3f, 0.3f, 1));
    g_pAmbientLight->SetDiffuseColor(XMFLOAT4(1.0f, 1.0f, 1.0f, 1));
    g_pAmbientLight->SetSpecularColor(XMFLOAT4(1, 1, 1, 1));

    //Light view
    XMVECTOR vLightPos = XMLoadFloat3(&v3Pos);
    XMVECTOR vLightDir = XMVectorNegate(XMVector3Normalize(vLightPos));

    g_matLightView = XMMatrixLookAtLH(vLightPos, vLightPos + vLightDir, XMVectorSet(0, 1, 0, 0));
    g_pAmbientLight->SetLightViewMatrix(g_matLightView);
    //Worker thread
...
void ShutdownSystem()
{
    ...
    if (g_pTextureSmoothShader)
    {
        delete g_pTextureSmoothShader;
        g_pTextureSmoothShader = nullptr;
    }

    if (g_pStencilShader)
    {
        delete g_pStencilShader;
        g_pStencilShader = nullptr;
    }
...
void Progress()
{
    ...
    g_v3CamPos = g_pCamera[(int)g_CameraMode]->GetPosition();
...
void Render()
{
    ...
    bStencilFillShow = g_bPostStencilFillShow;
    iFace = g_iRenderFaceCount;
    pBack = CWicBitmap::ConvertWicToD2dBitmap(dfRENDERTARGET(), g_pD2dSystem->GetBackbuffer());
    if(bZFillShow || bStencilFillShow) pZFill = CWicBitmap::ConvertWicToD2dBitmap(dfRENDERTARGET(), g_ZFillBuffer[(g_iZFill + 1)%2]->GetWicBitmap());
...
void RenderGeometry()
{
    ...
    bStencilFillShow = g_bStencilFillShow;
    ...
    if (lstRenderObj.size() != 0)
    {
        g_iFaceCount = 0;
        //For phong shading
        g_pAmbientLight->SetCameraPosition(v3CamPos);
        ...
        g_pTextureSmoothShader->SetWorldMatrix(g_matRenderWorld);
        g_pTextureSmoothShader->SetViewMatrix(g_matRenderView);
        g_pTextureSmoothShader->SetProjectionMatrix(g_matRenderProj);
        g_pTextureSmoothShader->SetViewportMatrix(g_matRenderViewport);
        ...
        g_pTextureShader->Initialize(g_pD2dSystem->GetRenderbuffer(), g_vecZBuffer.data(), g_vecStencilBuffer.data());
        g_pTextureSmoothShader->Initialize(g_pD2dSystem->GetRenderbuffer(), g_vecZBuffer.data(), g_vecStencilBuffer.data());

        //Stencil
        if (g_bStencil)
        {
            g_pStencilShader->Initialize(nullptr, nullptr, g_vecStencilBuffer.data());

            g_pStencilShader->SetWorldMatrix(g_matRenderWorld);
            //조명 방향으로의 뷰 설정
            g_pStencilShader->SetViewMatrix(g_matLightView);
            g_pStencilShader->SetProjectionMatrix(g_matRenderProj);
            g_pStencilShader->SetViewportMatrix(g_matRenderViewport);

            list<CGameObject*>::iterator iter;
            for (iter = lstRenderObj.begin(); iter != lstRenderObj.end(); ++iter)
            {
                //후면 렌더링
                StencilRenderObject(*iter, g_pStencilShader, CULL_BACKFACE::CW);
            }
        }
        ...
        for (iter = lstRenderObj.begin(); iter != lstRenderObj.end(); ++iter)
        {
            if (g_RenderState == RENDER_STATE::TEST || g_RenderState == RENDER_STATE::FLAT)
                RenderObject(*iter, g_pVtxColorShader, g_pAmbientLight);
            else if (g_RenderState == RENDER_STATE::TEXTURE && g_bFastComputeLight == false)
                RenderObject(*iter, g_pTextureSmoothShader, g_pAmbientLight);
            else if(g_RenderState == RENDER_STATE::TEXTURE && g_bFastComputeLight == true)
                RenderObject(*iter, g_pTextureShader, g_pAmbientLight);
        }
        ...
        else if (bStencilFillShow)
        {
            FillZValue(g_ZFillBuffer[g_iZFill]->GetWicBitmap(), g_vecStencilBuffer.data(), SCREEN_BUFFER_WIDTH, SCREEN_BUFFER_HEIGHT, 1.f);
        }
    ...
    g_bPostStencilFillShow = bStencilFillShow;
...
void BackfaceCulling(CMesh* pMesh, const CULL_BACKFACE enFaceWhere, list<CFace>& lstClipOut, const XMMATRIX& matWorld, const XMMATRIX& matView, const XMMATRIX& matProj)
{
    ...
    */
    ...
    XMMATRIX matFrustumView = XMMatrixMultiply(matMW, matView);
    XMMATRIX matMWVP = XMMatrixMultiply(matFrustumView, matProj);
    ...
    for (size_t model_cn = 0; model_cn < tModelCount; ++model_cn)
    {
        ...
        for (size_t cn = 0; cn < tFaceCount; cn++)
        {
            ...
            cullFace.MakeNormal();
            float fD = cullFace.CalcNormalDot(vLook);
...
void StencilRenderObject(CGameObject* pObj, CShader* pShader, const CULL_BACKFACE cull)
{
    ...
    if (pModel)
    {
        ...
        {
            pShader->SetWireframe(true);
            StencilDrawMesh(pModel, pShader, cull);
        }
...
void DrawMesh(CMesh* pMesh, CShader* pShader)
{
    ...
    if (lstClipFacies.size())
    {
        ...
        for (iter = lstClipFacies.begin(); iter != lstClipFacies.end(); ++iter)
        {
            ...
            //Vertex color shader
            //Light shading
            pShader->Render(&(*iter), pMesh->GetMaterial(), g_pAmbientLight, g_RenderState, g_bLightOn, g_bZBuffer, g_bStencil);
        }

    }
}

void StencilDrawMesh(CMesh* pMesh, CShader* pShader, const CULL_BACKFACE cull)
{
    XMMATRIX matModel = pMesh->GetModelMatrix();
    ...
    BackfaceCulling(pMesh, cull, lstClipFacies, g_matRenderWorld, g_matLightView, g_matRenderProj);
    ...
    if (lstClipFacies.size())
    {
        ...
        for (iter = lstClipFacies.begin(); iter != lstClipFacies.end(); ++iter)
        {
            pShader->Render(&(*iter), nullptr, nullptr, g_RenderState, false, false, true);
        }
...
void DrawBoundbox(CMesh* pMesh, CShader* pShader)
{
    ...
    if (lstClipFacies.size())
    {
        ...
        for (iter = lstClipFacies.begin(); iter != lstClipFacies.end(); ++iter)
        {
            ...
            g_iFaceCount++;
            pShader->Render(&(*iter), nullptr, nullptr, RENDER_STATE::BOUNDBOX, false, g_bZBuffer);
        }
