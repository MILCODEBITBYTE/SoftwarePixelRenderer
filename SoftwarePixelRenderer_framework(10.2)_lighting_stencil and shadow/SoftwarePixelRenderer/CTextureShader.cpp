...
void CTextureShader::Render(CFace* pFace, CMaterial* pMaterial, CLight *pLight,  const RENDER_STATE state, const bool bLight, const bool bZTest, const bool bStencil)
{
	...
	for (size_t i = 0; i < tVtxCnt; ++i)
	{
		...
		vRHW = XMVectorReplicate(v4Vtx.w);
		vUV = XMLoadFloat3(&pVertices[i].v3UV);
		vUV = XMVectorDivide(vUV, vRHW);
		XMStoreFloat3(&pVertices[i].v3UV, vUV);
	...
	if (tVtxCnt == 3)
		DrawTriangle(pVertices, pMaterial, pLight, pFace->GetMaterialName(),state, bLight, bZTest, bStencil);
	else
		DrawQuad(pVertices, pMaterial, pLight, pFace->GetMaterialName(),state, bLight, bZTest, bStencil);
}

void CTextureShader::DrawTriangle(VERTEX* pVertices, CMaterial* pMaterial, CLight *pLight, const wstring strMaterialName,const RENDER_STATE state, const bool bLight, const bool bZTest, const bool bStencil)
{
	if (nullptr == m_pScreenBuffer) return;
...
void CTextureShader::DrawQuad(VERTEX* pVertices, CMaterial* pMaterial, CLight *pLight, const wstring strMaterialName, const RENDER_STATE state, const bool bLight, const bool bZTest, const bool bStencil)
{
	if (nullptr == m_pScreenBuffer) return;
...
void CTextureShader::FillTrilinearInterpolation(BYTE* pBack, const UINT uiStride, const VERTEX* pVertices[3], CWicBitmap* pTex, CLight* pLight, const bool bLight, const bool bZTest, const bool bStencil)
{
	...
	function<void(CBresenhamLine&, const WORD, const XMFLOAT3&, const XMFLOAT3&, const float, const float)> fcLineNextStep = [&](CBresenhamLine& line, const WORD num, const XMFLOAT3& v3StartUV, const XMFLOAT3& v3EndUV, const float fStartZ, const float fEndZ)
	{
		...
		do
		{
			...
			XMStoreFloat4(&dir.UVZ, vFinalUVZ);
	...
	for (size_t step = 0; step < size; ++step)
	{
		...
		if (x < ptB.x)
		{
			...
			v4StartUVZ = vecTrilinear[step].interpolation[0].UVZ;
		}
		//part AB
		else
		{
			lStartY = vecTrilinear[step].interpolation[1].y;
			v4StartUVZ = vecTrilinear[step].interpolation[1].UVZ;
		}

		//part B
		lEndY = vecTrilinear[step].interpolation[2].y;
		v4EndUVZ = vecTrilinear[step].interpolation[2].UVZ;
...
void CTextureShader::FillLinearInterpolation(BYTE* pBack, const UINT uiStride, const VERTEX* pStart, const VERTEX* pEnd, CWicBitmap* pTex, CLight* pLight, const bool bLight, const bool bZTest, const bool bStencil)
{
	int w = int(m_matViewport._11 * 2.f);
	int h = int(m_matViewport._22 * -2.f);
	int half_w = w / 2;
	...
	do
	{
		...
		vStartUVZ = XMVectorSet(pStart->v3UV.x, pStart->v3UV.y, pStart->v3UV.z, pStart->v3Vtx.z);
		vEndUVZ = XMVectorSet(pEnd->v3UV.x, pEnd->v3UV.y, pEnd->v3UV.z, pEnd->v3Vtx.z);
