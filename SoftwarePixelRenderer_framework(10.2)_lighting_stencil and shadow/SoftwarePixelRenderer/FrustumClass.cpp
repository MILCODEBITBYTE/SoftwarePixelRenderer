...
VERTEX CFrustumClass::IntersectPlane(XMFLOAT4& v4Plane, VERTEX* vtxStart, VERTEX* vtxEnd)
{
	...
	vRate = XMVectorReplicate(fRate);

	vIntersect = XMVectorLerpV(vStart, vEnd, vRate);
	...
	//Color
	XMVECTOR vColStart, vColEnd, vColFinal;
	vColStart = XMLoadFloat4(&vtxStart->v4Color);
	vColEnd = XMLoadFloat4(&vtxEnd->v4Color);

	vColFinal = XMVectorLerpV(vColStart, vColEnd, vRate);
	XMStoreFloat4(&vtxNew.v4Color, vColFinal);
	
	//Texture UV
	XMVECTOR vUvStart, vUvEnd, vUvFinal;
	vUvStart = XMLoadFloat3(&vtxStart->v3UV);
	vUvEnd = XMLoadFloat3(&vtxEnd->v3UV);
	vUvFinal = XMVectorLerpV(vUvStart, vUvEnd, vRate);
	XMStoreFloat3(&vtxNew.v3UV, vUvFinal);
