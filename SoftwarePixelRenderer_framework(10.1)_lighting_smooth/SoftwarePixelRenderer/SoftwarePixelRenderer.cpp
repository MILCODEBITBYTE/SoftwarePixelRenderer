...
BOOL InitializeSystem()
{
    ...
    XMFLOAT3 v3Dir = XMFLOAT3(0, 0, 1);
    XMVECTOR vRotate = XMQuaternionRotationRollPitchYaw(-0.6f, XM_PI * 0.8f, 0);
    vRotate = XMVector3Rotate(XMVectorSet(0, 0, 1, 0), vRotate);
    vRotate = XMVector3Normalize(vRotate);
    XMStoreFloat3(&v3Dir, vRotate);

    g_pAmbientLight->SetDirection(v3Dir);
    g_pAmbientLight->SetPosition(XMFLOAT3(0, 5, -5));
...
void Progress()
{
    ...
    XMVECTOR vTest = XMVectorSet(1, 1, 0, 0);
    XMVECTOR vUnVector, vDet;

    XMMATRIX matWVPV, matUnW;
    matWVPV = XMMatrixMultiply(matView, matProj);
    matWVPV = XMMatrixMultiply(matWVPV, matViewport);

    vDet = XMMatrixDeterminant(matWVPV);
    matUnW = XMMatrixInverse(&vDet, matWVPV);

    vTest = XMVector3TransformCoord(vTest, matWVPV);
    vUnVector = XMVector3TransformCoord(vTest, matUnW);
...
void RenderGeometry()
{
    ...
    if (lstRenderObj.size() != 0)
    {
        ...
        g_pTextureSmoothShader->Initialize(g_pD2dSystem->GetRenderbuffer(), g_vecZBuffer.data());
...
void DrawMesh(CMesh* pMesh, CShader* pShader)
{
    ...
    if (lstClipFacies.size())
    {
        ...
        for (iter = lstClipFacies.begin(); iter != lstClipFacies.end(); ++iter)
        {
            ...
            //Vertex color shader
            //Light shading
            pShader->Render(&(*iter), pMesh->GetMaterial(), g_pAmbientLight, g_RenderState, g_bLightOn, g_bZBuffer);
        }
