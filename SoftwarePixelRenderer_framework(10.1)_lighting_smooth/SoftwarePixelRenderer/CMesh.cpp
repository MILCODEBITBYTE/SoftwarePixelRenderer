...
bool CMesh::LoadFromFile(const wchar_t* szPath, const wchar_t* szFileName)
{
    wstring strPathName;
    strPathName = wstring(szPath) + wstring(szFileName);

    CModelImportor imp(strPathName.c_str());
...
void CMesh::MakeMesh(CModelImportor* pImportor, const wchar_t* szPath)
{
    Shutdown();
    ...
    m_mtlMaterial.Open(szPath, pImportor->GetMaterialFileName());
}
    ...
    else
    {
        ...
        for (size_t i = 0; i < tFaceCount; ++i)
        {
            for (int cnt = 0; cnt < pFacies[i].NumOfElements; ++cnt)
            {
                ...
                //update texture
                newVertices[cnt]->v3UV.x = pUV[idxUV].u;
                newVertices[cnt]->v3UV.y = pUV[idxUV].v;
                newVertices[cnt]->v3UV.z = 1;
