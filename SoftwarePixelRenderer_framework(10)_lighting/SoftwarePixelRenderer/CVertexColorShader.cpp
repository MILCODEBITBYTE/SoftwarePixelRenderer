...
void CVertexColorShader::Render(CFace* pFace, CMaterial* pMaterial, CLight *pLight, const RENDER_STATE state, const bool bLight, const bool bZTest)
{
	...
	if (tVtxCnt == 3)
		DrawTriangle(pVertices, nullptr, pLight, pFace->GetMaterialName(),state, bLight, bZTest);
	else
		DrawQuad(pVertices,nullptr, pLight, pFace->GetMaterialName(), state, bLight, bZTest);
}


void CVertexColorShader::DrawTrilinearInterpolation(BYTE* pBack, const UINT stride, const VERTEX* pVertices[3], CLight *pLight, const bool bLight, const bool bZTest)
{
...
void CVertexColorShader::DrawLinearInterpolation(BYTE* pBuffer, const UINT uiStride, const VERTEX* pStart, const VERTEX* pEnd, CLight* pLight,const bool bLight, const bool bZTest)
{
	int w = int(m_matViewport._11 * 2.f);
	int h = int(m_matViewport._22 * -2.f);
...
void CVertexColorShader::FillTrilinearInterpolation(BYTE* pBack, const UINT stride, const VERTEX* pVertices[3], CLight* pLight,  const bool bLight, const bool bZTest)
{
	int w = int(m_matViewport._11 * 2.f);
	int h = int(m_matViewport._22 * -2.f);
...
void CVertexColorShader::FillLinearInterpolation(BYTE* pBuffer, const UINT uiStride, const VERTEX* pStart, const VERTEX* pEnd, CLight* pLight, const bool bLight, const bool bZTest)
{
	int w = int(m_matViewport._11 * 2.f);
	int h = int(m_matViewport._22 * -2.f);
...
void CVertexColorShader::DrawTriangle(VERTEX* pVertices, CMaterial* pMaterial, CLight *pLight, const wstring strMaterialName, const RENDER_STATE state, const bool bLight, const bool bZTest)
{
	if (nullptr == m_pScreenBuffer) return;
	...
	*/
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			{
				DrawLinearInterpolation(pBackData, uiBackStride, pVertices, pVertices + 1, pLight, bLight, bZTest);
				DrawLinearInterpolation(pBackData, uiBackStride, pVertices + 1, pVertices + 2, pLight, bLight, bZTest);
				DrawLinearInterpolation(pBackData, uiBackStride, pVertices + 2, pVertices, pLight, bLight, bZTest);
			}
			...
			else if (state == RENDER_STATE::TEST)
			{
				...
				{
					DrawTrilinearInterpolation(pBackData, uiBackStride, Tri, pLight, bLight, bZTest);
				}
				else
				{
					DrawLinearInterpolation(pBackData, uiBackStride, pVertices, pVertices + 1, pLight, bLight, bZTest);
			...
			else if (state == RENDER_STATE::FLAT)
			{
				if (false == m_bWireframe)
				{
					FillTrilinearInterpolation(pBackData, uiBackStride, Tri, pLight, bLight, bZTest);
				}
				else
				{
					FillLinearInterpolation(pBackData, uiBackStride, pVertices, pVertices + 1, pLight, bLight, bZTest);
					FillLinearInterpolation(pBackData, uiBackStride, pVertices+1, pVertices + 2, pLight, bLight, bZTest);
					FillLinearInterpolation(pBackData, uiBackStride, pVertices+2, pVertices, pLight, bLight, bZTest);
				}
			}
...
void CVertexColorShader::DrawQuad(VERTEX* pVertices, CMaterial* pMaterial, CLight *pLight, const wstring strMaterialName, const RENDER_STATE state, const bool bLight, const bool bZTest)
{
	if (nullptr == m_pScreenBuffer) return;
	...
	*/
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (state == RENDER_STATE::BOUNDBOX)
			{
				DrawLinearInterpolation(pBackData, uiBackStride, TriA[0], TriA[1], pLight, bLight, bZTest);
				DrawLinearInterpolation(pBackData, uiBackStride, TriA[1], TriA[2], pLight, bLight, bZTest);
				DrawLinearInterpolation(pBackData, uiBackStride, TriA[2], TriA[0], pLight, bLight, bZTest);

				DrawLinearInterpolation(pBackData, uiBackStride, TriB[0], TriB[1], pLight, bLight, bZTest);
				DrawLinearInterpolation(pBackData, uiBackStride, TriB[1], TriB[2], pLight, bLight, bZTest);
				DrawLinearInterpolation(pBackData, uiBackStride, TriB[2], TriB[0], pLight, bLight, bZTest);
			}
			else if (state == RENDER_STATE::TEST)
			{
				if (false == m_bWireframe)
				{
					DrawTrilinearInterpolation(pBackData, uiBackStride, TriA, pLight, bLight, bZTest);
					DrawTrilinearInterpolation(pBackData, uiBackStride, TriB, pLight, bLight, bZTest);
				}
				else
				{
					DrawLinearInterpolation(pBackData, uiBackStride, TriA[0], TriA[1], pLight, bLight, bZTest);
			...
			else if (state == RENDER_STATE::FLAT)
			{
				if (false == m_bWireframe)
				{
					FillTrilinearInterpolation(pBackData, uiBackStride, TriA, pLight, bLight, bZTest);
					FillTrilinearInterpolation(pBackData, uiBackStride, TriB, pLight, bLight, bZTest);
				}
				else
				{
					FillLinearInterpolation(pBackData, uiBackStride, TriA[0], TriA[1], pLight, bLight, bZTest);
					FillLinearInterpolation(pBackData, uiBackStride, TriA[1], TriA[2], pLight, bLight, bZTest);
					FillLinearInterpolation(pBackData, uiBackStride, TriA[2], TriA[0], pLight, bLight, bZTest);

					FillLinearInterpolation(pBackData, uiBackStride, TriB[0], TriB[1], pLight, bLight, bZTest);
					FillLinearInterpolation(pBackData, uiBackStride, TriB[1], TriB[2], pLight, bLight, bZTest);
					FillLinearInterpolation(pBackData, uiBackStride, TriB[2], TriB[0], pLight, bLight, bZTest);
