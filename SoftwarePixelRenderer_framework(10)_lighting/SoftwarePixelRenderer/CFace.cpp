...
void CFace::SetFace(const VERTEX* pVertices, const int iVtxCount)
{
	if (m_iFaceInVertexCount != iVtxCount)
	{
		m_vecVertices.resize(iVtxCount);
		m_iFaceInVertexCount = iVtxCount;
	}
	for (int i = 0; i < iVtxCount; ++i)
		m_vecVertices[i] = pVertices[i];
}
...
XMVECTOR CFace::MakeNormal(const XMFLOAT3* pVertices)
{
	XMVECTOR p1, p2;
	XMFLOAT3 v3Dir1, v3Dir2;
	v3Dir1 = pVertices[0] - pVertices[1];
	v3Dir2 = pVertices[2] - pVertices[1];
	p1 = XMLoadFloat3(&v3Dir1);
	p2 = XMLoadFloat3(&v3Dir2);

	return XMVector3Normalize(XMVector3Cross(p1, p2));
}
...
float CFace::CalcNormalDot(const XMFLOAT3& v3)
{
	XMFLOAT3& v3Normal = m_vecVertices[0].v3Normal;

	return v3Normal.x * v3.x + v3Normal.y * v3.y + v3Normal.z * v3.z;
}

float CFace::CalcNormalDot(const XMVECTOR& v)
{
	XMFLOAT3 v32;
	...
	XMStoreFloat3(&v32, v);
	return v3Normal.x * v32.x + v3Normal.y * v32.y + v3Normal.z * v32.z;
}

wstring CFace::GetMaterialName() const
{
	return m_strMaterialName;
}

void CFace::SetMaterialName(const wstring strMaterialName)
{
